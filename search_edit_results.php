	...
	<div id="content-outer" class="clear">
		...
		<div class="content">
                    ...
                    <?php
                        ...
                        {
                            echo "<form name= 'DeleteForm" . $br . "' method='post'
                            action='edit.php'  id='RentForm".$br."'>";
                            ...
                            echo "<input type='hidden' id='ADD' name='ADDRESS'
                                   value='".$row['Address']."' />";
                            echo "<input type='hidden' id='CIT' name='CITY'
                                   value='".$row['city']."' />";
                            echo "<input type='hidden' id='ARE' name='AREA'
                                   value='".$row['Area']."' />";
                            echo "<input type='hidden' id='FLO' name='FLO'
                                   value='".$row['Floor']."' />";

                            echo "<b>Тип на жилище:</b> ";
                            $query2 = "SELECT NameOfType FROM type_of_house 
                                       WHERE TypeOfHouse_PK = ".
                                       $row['TypeOfHouse_FK'];
                            ...
                            while($row2 = mysql_fetch_array($result2))
                            {
                                ...
                                $NOT = $row2['NameOfType'];
                                echo "<input type='hidden' id='NOT'
                                    name='TIP_J' value='".$NOT."' />";
                            }
                            ...
                            else
                            {
                                echo "незаето <br>";
                                $STATUS = "незаето";
                                echo "<input type='hidden' id='STA' name='STA'
                                    value='".$STATUS."' />";
                            }

                            echo "<b>Тип на обзавеждане:</b> ";
                            $query3 = "SELECT NameOfType FROM type_of_furnishing 
                                WHERE TypeOfFurnishing_PK = ".
                                $row['TypeOfFurnishing_FK'];
                            $result3 = mysql_query ($query3);
                            while($row3 = mysql_fetch_array($result3))
                            {
                                echo $row3['NameOfType'];
                                $FURN_TYPE = $row3['NameOfType'];
                                echo "<input type='hidden' id='FURN' name='FURN'
                                    value='".$FURN_TYPE."' />";
                            }

                            echo "<br><b>Месечен наем: </b>". $row['MonthRent']
                                    . "<br>";
                            echo "<input type='hidden' id='MRENT' name='MRENT'
                                value='".$row['MonthRent']."' />";

                            echo "<b>Наемодател:</b> ";
                            $query4 = "SELECT Name FROM landlordtable WHERE
                                LandLord_PK  = ".$row['LandLord_FK'];
                            $result4 = mysql_query ($query4);
                            while($row4 = mysql_fetch_array($result4))
                            {
                                echo $row4['Name'];
                                $LNDLORD = $row4['Name'];
                                echo "<input type='hidden' id='LLORD'
                                    name='LLORD' value='".$LNDLORD."' />";
                            }

                            echo "<br><b>Местоположение:</b> ";
                            $query5 = "SELECT Position FROM placement_category 
                                WHERE PlacementCategory_PK  = ".
                                $row['PlacementCategory_FK'];
                            $result5 = mysql_query ($query5);
                            while($row5 = mysql_fetch_array($result5))
                            {
                                echo $row5['Position'];
                                $POSITION = $row5['Position'];
                                echo "<input type='hidden' id='POS' name='POS'
                                    value='".$POSITION."' />";
                            }

                            echo "<br><b>Допълнителни коментари: </b>" .
                                    $row['comment']. "<br>";
                            ...
                            echo " <input class='button' type='submit'
                                name='edit".$br."' value='Редактирай!'/>";
                            echo "</form>";
                        }

                        if ($br == 0)
                            echo "<p><center><h3>Няма резултати от
                                търсенето!</p>";
                        ?>
	...
	<div id="footer-bottom">
        <p>
			...
			<a href="http://www.bluewebtemplates.com/" 
                           title="Website Templates">website templates</a> by
